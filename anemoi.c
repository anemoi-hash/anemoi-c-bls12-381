...
#define PARAMETERS_DELTA_UINT64_1 15811494917868205788ul
#define PARAMETERS_DELTA_UINT64_2 16624336857528583898ul
#define PARAMETERS_DELTA_UINT64_3 17920762862402342329ul
#define PARAMETERS_DELTA_UINT64_4 7408687358795335574ul

#define PARAMETERS_PI_0_UINT64_1 9577757860007206479ul
#define PARAMETERS_PI_0_UINT64_2 9722245304449209307ul
#define PARAMETERS_PI_0_UINT64_3 738442708298218386ul
#define PARAMETERS_PI_0_UINT64_4 1897098738131453135ul

#define PARAMETERS_PI_1_UINT64_1 5115160819720334814ul
#define PARAMETERS_PI_1_UINT64_2 6500052438371763032ul
...
void blst_fr_set_to_one(blst_fr *r) {
  ...
  blst_scalar_from_uint64(&s, t);
  blst_fr_from_scalar(r, &s);
}

void blst_fr_mul_by_g(blst_fr *res, blst_fr *v) {
  blst_fr tmp;
...
void blst_fr_penta_pow(blst_fr *res, blst_fr *x) {
  blst_fr_mul(res, x, x);
  blst_fr_mul(res, res, res);
  blst_fr_mul(res, res, x);
}

void anemoi_compute_round_constants(blst_fr *buffer, int l, int nb_rounds) {
  blst_fr pi_0;
  uint64_t pi_0_uint64[4] = {PARAMETERS_PI_0_UINT64_1, PARAMETERS_PI_0_UINT64_2,
  ...
  uint64_t pi_1_uint64[4] = {PARAMETERS_PI_1_UINT64_1, PARAMETERS_PI_1_UINT64_2,
                             PARAMETERS_PI_1_UINT64_3,
                             PARAMETERS_PI_1_UINT64_4};
  blst_fr_from_uint64(&pi_1, pi_1_uint64);

  blst_fr delta;
  uint64_t delta_uint64[4] = {
      PARAMETERS_DELTA_UINT64_1, PARAMETERS_DELTA_UINT64_2,
  ...
  blst_fr pi_0_i;
  blst_fr_set_to_one(&pi_0_i);
  for (int i = 0; i < nb_rounds; i++) {
    blst_fr pi_0_i_square;
    blst_fr_mul(&pi_0_i_square, &pi_0_i, &pi_0_i);

    blst_fr pi_1_j;
    blst_fr pi_1_j_square;
    blst_fr pi_0_i_plus_pi_1_j;
    blst_fr tmp;
    blst_fr tmp_prime;
    blst_fr_set_to_one(&pi_1_j);
    for (int j = 0; j < l; j++) {
      // pi_1_j^2
      blst_fr_mul(&pi_1_j_square, &pi_1_j, &pi_1_j);
      // pi_0_i + pi_1_j
      blst_fr_add(&pi_0_i_plus_pi_1_j, &pi_0_i, &pi_1_j);
      // (pi_0_i + pi_1_j)^5
      blst_fr_penta_pow(&tmp, &pi_0_i_plus_pi_1_j);
      // g * pi_0_i_square
      blst_fr_mul_by_g(&tmp_prime, &pi_0_i_square);
      // csts_c[i * l + j] = g * pi_0_i_square +
      //                   (pi_0_i + pi_1_j)^5
      blst_fr_add(csts_c + (i * l) + j, &tmp_prime, &tmp);

      // g * pi_1_j_square
      blst_fr_mul_by_g(&tmp_prime, &pi_1_j_square);
      // csts_d[i * l + j] = g * pi_1_j_square +
      //                   (pi_0_i + pi_1_j)^5
      blst_fr_add(csts_d + (i * l) + j, &tmp_prime, &tmp);

      // for pow
      blst_fr_mul(&pi_1_j, &pi_1_j, &pi_1);
    }

    // for pow
    blst_fr_mul(&pi_0_i, &pi_0_i, &pi_0);
  }
}
...
void anemoi_2_apply_linear_layer(anemoi_ctxt_t *ctxt) {
  ...
  // Apply M_x
  // Compute "g * y' and save it in tmp.
  blst_fr_mul_by_g(&tmp, state_x + 1);
  // x += g * y. Inplace operation
  blst_fr_add(state_x, state_x, &tmp);

  // Compute "g * x' and save it in tmp.
  blst_fr_mul_by_g(&tmp, state_x);
  ...
  // Apply M_y
  // Compute "g * y' and save it in tmp.
  blst_fr_mul_by_g(&tmp, state_y + 1);
  // x += g * y. Inplace operation
  blst_fr_add(state_y, state_y, &tmp);

  // Compute "g * x' and save it in tmp.
  blst_fr_mul_by_g(&tmp, state_y);
...
void anemoi_4_apply_matrix(blst_fr *ctxt) {
  ...
  // x[3] += g x[0]
  blst_fr_mul_by_g(&tmp, ctxt);
  blst_fr_add(ctxt + 3, ctxt + 3, &tmp);
  // x[1] = g * (x[1] + x[2])
  blst_fr_add(&tmp, ctxt + 1, ctxt + 2);
  blst_fr_mul_by_g(ctxt + 1, &tmp);
  // x[0] += x[1]
  blst_fr_add(ctxt, ctxt, ctxt + 1);
  // x[2] += g x[3]
  blst_fr_mul_by_g(&tmp, ctxt + 3);
...
anemoi_ctxt_t *anemoi_allocate_context(int l, int nb_rounds) {
  ...
  int nb_fr_elements = l * nb_rounds * 2;
  ...
  blst_fr *state = malloc(sizeof(blst_fr) * nb_fr_elements);
  if (state == NULL) {
    free(ctxt);
    return (NULL);
  }

  anemoi_compute_round_constants(state, l, nb_rounds);
